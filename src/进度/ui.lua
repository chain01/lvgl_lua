...
local function create()
    ...
    lvgl.obj_set_pos(btn1, 5, 5) --设置坐标
    ...
    lvgl.obj_set_size(btn2, 30, 20) --设置大小
    lvgl.obj_align(btn2, btn1, lvgl.ALIGN_OUT_RIGHT_TOP, 5, 0) --设置对齐方式
    ...
    btn2_label = lvgl.label_create(btn2, nil) --给 btn2 添加 label 子对象
    ...
    --3.创建一个宽度自适应的正常按钮
    btn3 = lvgl.btn_create(scr0, nil)
    lvgl.obj_align(btn3, btn1, lvgl.ALIGN_OUT_BOTTOM_LEFT, 0, 5) --设置对齐
    ...
    my_style2 = lvgl.style_t()
    lvgl.style_copy(my_style2, lvgl.style_plain_color)
    my_style2.body.main_color = lvgl.color_hex(0xbbbbbb)
    my_style2.body.grad_color = lvgl.color_hex(0xbbbbbb)
    my_style2.body.radius = lvgl.RADIUS_CIRCLE --绘制圆角
    --1.2 创建指示器样式
    my_style3 = lvgl.style_t()
    lvgl.style_copy(my_style3, lvgl.style_plain_color)

    my_style3.body.main_color = lvgl.color_hex(0x5fb878)
    my_style3.body.grad_color = lvgl.color_hex(0x5fb878)
    my_style3.body.radius = lvgl.RADIUS_CIRCLE --绘制圆角
    my_style3.body.padding.left = 0 --让指示器跟背景边框之间没有距离
    my_style3.body.padding.top = 0
    my_style3.body.padding.right = 0
    my_style3.body.padding.bottom = 0
    --2.创建水平进度条
    bar1 = lvgl.bar_create(scr0, nil) --创建进度条
    lvgl.obj_set_size(bar1, 180, 16) --设置大小,宽度比高度大就是水平的
    lvgl.obj_set_pos(bar1, 20, 180) --设置坐标
    lvgl.bar_set_style(bar1, lvgl.BAR_STYLE_BG, my_style2) --设置进度条背景的样式
    --设置进度条指示器的样式
    lvgl.bar_set_style(bar1, lvgl.BAR_STYLE_INDIC, my_style3)
    lvgl.bar_set_anim_time(bar1, 1000) --设置动画时长
    lvgl.bar_set_value(bar1, 100, lvgl.ANIM_ON) --设置新的进度值,带有动画效果的
